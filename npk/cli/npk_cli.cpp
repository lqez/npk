...
void sync_only_in_package( bool sd, bool force, bool usefilesize, const char* path );
    ...
    else
    {
        ...
        else if( V(2,"sync") || V(2,"-sync") || V(2,"update") || V(2,"-update") )
        {
            cout << "sync/update: Synchronize the package with local files.\n"
                ...
                << "    --fs [--filesize]      : Use filesize for comparison.\n"
...
void get_sync_options()
{
    ...
    filesize = find_option( "fs" ) | find_option( "filesize" );
}
...
void flag()
{
    ...
    while( n < c )
    {
        ...
        strcpy( entityname, v[n] );
        char *flagchar = NULL;
        NPK_FLAG flag = NPK_ENTITY_REVERSE; // for better compression
...
void sync_package()
{
    ...
    sync_only_in_package( syncdelete, forceupdate, filesize, path );
    ...
    while( eb )
    {
        ...
        if ( status == 0 )
        {
            ...
            else
            {
                ...
                {
                    if( usefilesize && __sbuf.st_size != eb->info_.originalSize_ )
                    {
                        status = 2;
                    }
